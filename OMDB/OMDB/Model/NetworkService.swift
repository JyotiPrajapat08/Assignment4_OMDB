...
extension NetworkService {
    ...
    func search(for name: String, page: Int = 1, handler: @escaping(JSON.Search?, Error?) -> Void){
        DispatchQueue.main.async {
            ...
            self.request(url: EndPoints.Search.path, httpMethod: .get, headers: nil, body: nil, parameters: queries){
    ...
    func getImage(url: String, handler: @escaping(Data?, Error?) -> Void){
        ...
        DispatchQueue.main.async {
            if let cachedData = self.cache.object(forKey: cacheID) {
                handler((cachedData as Data), nil)
            }else{
                if let url = URL(string: url) {
                    let session = URLSession(configuration: self.urlSessionConfig)
